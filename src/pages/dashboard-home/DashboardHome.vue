...
export default defineComponent({
  ...
  setup() {
    ...
    const subTextHome = ref(
      'Já imaginou um lugar onde a beleza ganha vida de verdade? A FireBeauty é mais do que um app, é a primeira rede social feita para conectar você aos melhores profissionais do mercado. Aqui, você descobre tendências, agenda serviços com facilidade e ainda constrói sua reputação no mundo da estética.',
    );
    const textCardForm = ref('Receba em primeira mão\num benefício exclusivo da\nFireBeauty!');
    const textRedeSocial = ref('REDE SOCIAL DA BELEZA');
    const subTextRedeSocial = ref(
      'Mais do que um app, a FireBeauty é uma comunidade!\nProfissionais criam perfis, compartilham seus trabalhos e\nconquistam novos clientes, enquanto usuários encontram\ninspirações e avaliam serviços com base em experiências reais.',
    );
    const containerFirstTitle = ref('Confiança e Qualidade');
    const containerFirstSubTitle = ref(
      'Avaliações reais, recomendações personalizadas e transparência total. Você escolhe com segurança, sabendo que cada profissional tem seu trabalho validado pela comunidade.',
    );
    const containerSecondTitle = ref('Transformamos a beleza!');
    const containerSecondSubTitle = ref(
      'Chegamos para digitalizar e simplificar o mercado da estética, conectando clientes e profissionais de forma rápida, prática e confiável. Seja para encontrar o serviço ideal ou expandir sua carreira, este é o lugar certo!',
    );
    const containerThirdTitle = ref('Agendamentos Simples e Rápidos');
    const containerThirdSubTitle = ref(
      'Nada de mensagens intermináveis! Os clientes agendam serviços diretamente no perfil dos profissionais, com horários atualizados e confirmação instantânea.',
    );
    const containerFourthTitle = ref('Oportunidade para Profissionais');
    const containerFourthSubTitle = ref(
      'Se você trabalha com beleza, a FireBeauty é o seu palco! Construa sua reputação, ganhe visibilidade e atraia mais clientes sem depender apenas do boca a boca.',
    );
